    ...
    for(int i = ja; i<=jb; i++){
      ...
      if(oacc > 2){
        ...
        if(oacc > 4){
          ...
          un[i] = unm1[i]
                + dt*def->g(x[i])
                + pow(sigma,2)/2/180*(2*unm1[i-3]-27*unm1[i-2]+270*unm1[i-1]-490*unm1[i]+270*unm1[i+1]-27*unm1[i+2]+2*unm1[i+3])
                + dt*pow(sigma,2)/6/180*(2*def->g(x[i-3])-27*def->g(x[i-2])+270*def->g(x[i-1])-490*def->g(x[i])+270*def->g(x[i+1])-27*def->g(x[i+2])+2*def->g(x[i+3]))
                + pow(sigma,4)/24/6*(-1*unm1[i-3]+12*unm1[i-2]-39*unm1[i-1]+56*unm1[i]-39*unm1[i+1]+12*unm1[i+2]-1*unm1[i+3])
                + dt*pow(sigma,4)/120/6*(-1*def->g(x[i-3])+12*def->g(x[i-2])-39*def->g(x[i-1])+56*def->g(x[i])-39*def->g(x[i+1])+12*def->g(x[i+2])-1*def->g(x[i+3]))
                + pow(sigma,6)/720*(1*unm1[i-3]-6*unm1[i-2]+15*unm1[i-1]-20*unm1[i]+15*unm1[i+1]-6*unm1[i+2]+1*unm1[i+3]);
        }
      }
    }
  }

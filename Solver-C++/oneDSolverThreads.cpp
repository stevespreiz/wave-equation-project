...
int main(int argc, char* argv[]){
  ...
  */
  ...
  thread firstL(firstStep,def, sigma, x, dt, ja, ja+oacc/2, unm1, un, nD, oacc);
  thread firstR(firstStep,def,sigma,x,dt,jb-oacc/2,jb,unm1,un,nD,oacc);
  thread firstM(firstStep,def,sigma,x,dt,ja+oacc/2+1,jb-oacc/2-1,unm1,un,nD,oacc);
  firstL.join();
  firstR.join();
  thread firstBC(BC,def,sigma,x,1,dt,ja,jb,un,nD,icase,oacc);
  firstM.join();
  firstBC.join();
  ...
  int n = 2;
  while(n*dt <= tf){
    thread timeL(timeStep,sigma,ja,ja+oacc/2,unm1,un,unp1,nD,oacc);
    thread timeR(timeStep,sigma,jb-oacc/2,jb,unm1,un,unp1,nD,oacc);
    thread timeM(timeStep,sigma,ja+oacc/2+1,jb-oacc/2-1,unm1,un,unp1,nD,oacc);
    timeL.join();
    timeR.join();
    thread timeBC(BC,def,sigma,x,n,dt,ja,jb,unp1,nD,icase,oacc);
    timeM.join();
    timeBC.join();
